import { examBank, type ExamQuestion, type ExamSection } from '@/seed/exam';
...
export function scoreExamAnswers(answers:AnswerMap, questions:readonly ExamQuestion[]=examBank){
  ...
  const review=questions.map(question=>{const correct=checkExamAnswer(question.section,question.answer,answers[question.id]??'');if(correct)sectionScores[question.section]+=question.points;return{id:question.id,section:question.section,correct};});
