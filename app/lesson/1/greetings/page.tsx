...
export default function GreetingsPage() {
  ...
  return <SiteShell><CommunityContextProvider context={{lessonId:1,section:'dialogues',concept:'greetings',skill:'communication',route:'/lesson/1/greetings'}}><main>
    ...
    <section className="dialogue-text greeting-list shell">{greetings.map((greeting) => <article key={greeting.id}><div><h2>{greeting.hanzi}</h2><p><PinyinText>{greeting.pinyin}</PinyinText></p><span>{greeting.translation}</span></div><SpeakButton text={greeting.hanzi} audioSrc={audioForMandarinText(greeting.hanzi)} /></article>)}</section>
