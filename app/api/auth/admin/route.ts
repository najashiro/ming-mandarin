...
import { isAdminUser, type AppUser } from '@/app/auth';
...
export async function POST(request: Request) {
  try {
    ...
    if (!isAdminUser(authUser)) return Response.json({ error: 'La cuenta no tiene permisos administrativos.' }, { status: 403 });
