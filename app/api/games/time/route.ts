...
async function leaderboard(userId: string) {
  const [rows, own] = await Promise.all([
    supabaseRest<RankRow[]>('time_game_leaderboard?select=rank,user_id,player_name,score&order=rank.asc&limit=100'),
    supabaseRest<RankRow[]>(`time_game_leaderboard?select=rank,user_id,player_name,score&user_id=eq.${userId}&limit=1`),
  ]);
...
export async function PUT() {
  try {
    return NextResponse.json({ token: issueTimeChallengeToken(supabaseSecretKey()) });
  } catch {
...
export async function POST(request: Request) {
  ...
  if (!session || body.correctAnswers! > 420 || body.maxStreak! > body.correctAnswers! || body.masteryBonusTotal! > body.correctAnswers! * 2 || body.score! > body.correctAnswers! * 17 || body.maxDifficulty! > Math.min(100, body.correctAnswers! * 6)) {
    return NextResponse.json({ error: 'El reto aún no ha terminado o el resultado no es válido.' }, { status: 400 });
  }
  try {
    await supabaseRest('time_game_sessions?on_conflict=id', { method: 'POST', prefer: 'resolution=ignore-duplicates,return=minimal', body: { id: session.id, user_id: user.userId, started_at: new Date(session.startedAt).toISOString() } });
    ...
    return NextResponse.json(await leaderboard(user.userId));
  } catch {
