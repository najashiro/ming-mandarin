...
export default async function ProgressPage({searchParams}:{searchParams:Promise<{scope?:string}>}) {
  ...
  return <SiteShell><main>
    ...
    <section className="shell hanzi-progress-section"><div className="section-heading"><div><p className="eyebrow"><Hanzi>汉字 · RUTA POR LECCIÓN Y TEXTO</Hanzi></p><h2>{data.hanziSummary.studied} / {data.hanziSummary.total} Hanzi estudiados</h2></div><Link href={hanziHref}>Abrir laboratorio →</Link></div><p className="source-note">“Estudiado” exige una práctica registrada; abrir una ficha no aumenta el contador.</p><div className="hanzi-dimension-summary">{data.hanziSummary.dimensions.map((item)=><article key={item.dimension}><span>{dimensionLabels[item.dimension]}</span><strong>{item.average}%</strong></article>)}</div><div className="hanzi-stage-summary">{data.hanziSummary.stages.map((stage)=><article key={stage.id}><span>{stage.id}</span><div><b><Hanzi>{stage.title}</Hanzi></b><small>{stage.studied} / {stage.total}</small><i><em style={{width:`${stage.total?stage.studied/stage.total*100:0}%`}}/></i></div></article>)}</div>{data.hanzi.length?<div className="hanzi-progress-grid">{data.hanzi.map((item)=><article key={`${item.characterId}-${item.skillDimension}`}><strong><Hanzi>{item.character}</Hanzi></strong><span>{dimensionLabels[item.skillDimension]??item.skillDimension}</span><b>{item.mastery}%</b></article>)}</div>:<div className="panel empty-state">Aún no hay intentos Hanzi sincronizados.</div>}</section>
  </main></SiteShell>;
}
