...
export default async function AdminContent({ searchParams }: { searchParams: Promise<{ source?: string }> }) {
  ...
  return <SiteShell><main>
    ...
    <div className="audit-table hanzi-audit-table shell"><div className="audit-head"><span><Hanzi>汉字</Hanzi></span><span>Pinyin</span><span>Significado</span><span>Introducido</span><span>Trazos</span><span>Aparece en</span><span>Rol / fuente</span><span>Escritura</span><span>Radical</span><span>Componentes</span><span>Datos</span></div>{filteredCharacters.map((item) => {
      ...
      return <div key={`hanzi-${item.id}`}><b><Hanzi>{item.hanzi}</Hanzi></b><span><PinyinText>{item.pinyin}</PinyinText></span><span><Hanzi>{item.meaning}</Hanzi></span><span>{item.introducedIn}</span><span>{technical?.strokeCount ?? '—'}</span><span>{item.appearsIn.join(' · ')}</span><span>{item.sourceRole}<small>{item.source.file} · PDF {item.source.pdfPage}</small></span><strong>{item.writingRequired ? 'Sí' : 'No'}</strong><span><Hanzi>{item.radicalAudited ? item.radical : 'No auditado'}</Hanzi></span><span><Hanzi>{item.componentsAudited ? item.components.join(' + ') : 'No auditados'}</Hanzi></span><strong className={technical?.available ? 'available' : 'unavailable'}>{technical?.available ? 'Sí' : 'No'}</strong></div>;
    })}</div>
  </main></SiteShell>;
}
