...
const report = { corpusFingerprint: corpus.fingerprint, publishedVocabulary: corpus.vocabulary.length, sourceVocabulary: knowledge.stats.vocabulary_entries, note: `${knowledge.stats.vocabulary_entries - corpus.vocabulary.length} fragmentos léxicos mal segmentados permanecen fuera de la exportación pública. Las palabras publicadas consumen pinyin y español resueltos; la elegibilidad curricular y las clasificaciones pendientes se conservan por separado.`, counts, exclusiveCatalog, canonicalLessons, pendingClassification, missingHanzi, audio: resources.counters, images: { pilot: media.length, approved: media.filter(m => m.status === 'approved' && m.src && existsSync(`public${m.src}`)).length, pending: media.filter(m => m.status !== 'approved').length } };
