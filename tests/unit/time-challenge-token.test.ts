...
describe('signed time challenge token', () => {
  it('accepts a completed seven minute challenge once the minimum duration has passed', () => {
    const start = 1_780_000_000_000;
    const token = issueTimeChallengeToken('test-secret', start, '123e4567-e89b-12d3-a456-426614174000');
    expect(verifyTimeChallengeToken(token, 'test-secret', start + 414_999)).toBeNull();
    expect(verifyTimeChallengeToken(token, 'test-secret', start + 415_000)).toEqual({ id: '123e4567-e89b-12d3-a456-426614174000', startedAt: start });
