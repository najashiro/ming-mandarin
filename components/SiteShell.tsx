  ...
  return (
    <>
      <header className="topbar">
        ...
        <Link className="profile-chip" href={user ? '/profile' : signInPath('/')}><span aria-hidden="true">学</span><b>{user ? user.displayName : 'Entrar'}</b></Link>
      </header>
      {children}
      <footer className="site-footer shell"><div><b>明 Míng</b><p>Lección 1 · aprendizaje persistente y verificable.</p></div><nav><Link href="/leaderboard">Ranking</Link><Link href="/errors">Errores</Link><Link href="/admin/content">Fuentes</Link>{user && <Link href={signOutPath('/')}>Salir</Link>}</nav></footer>
