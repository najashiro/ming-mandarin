...
export function VocabularyExplorer({ vocabulary, route = '/lesson/1/vocabulary', showSearch = vocabulary.length > 9 }: Props) {
  ...
  return <>
    ...
    <section className="vocab-grid shell">{visible.map((item)=><article className="vocab-card" key={item.id}><div className="vocab-actions"><button aria-label="Favorito" type="button" onClick={()=>setFavorites((values)=>values.includes(item.id)?values.filter(id=>id!==item.id):[...values,item.id])}>{favorites.includes(item.id)?'★':'☆'}</button></div><button type="button" className="hanzi-button" onClick={()=>setHidden(values=>values.includes(item.id)?values.filter(id=>id!==item.id):[...values,item.id])}><Hanzi>{item.hanzi}</Hanzi></button><p className="word-pinyin"><PinyinText>{item.pinyin}</PinyinText></p>{hidden.includes(item.id)?<p className="hidden-meaning">Significado oculto · toca el hanzi</p>:<p><Hanzi>{item.translation}</Hanzi></p>}<dl><div><dt>Clase</dt><dd>{item.grammaticalType}</dd></div>{item.example&&<div><dt>Ejemplo</dt><dd><Hanzi>{item.example}</Hanzi></dd></div>}</dl><div className="vocab-community-row"><SpeakButton text={item.hanzi} audioSrc={audioForMandarinText(item.hanzi)}/><CommunityButton compact label={`Preguntas sobre ${item.hanzi}`} context={{concept:item.hanzi,route:`${route}?concept=${encodeURIComponent(item.hanzi)}`}}/></div></article>)}</section>
  </>;
}
