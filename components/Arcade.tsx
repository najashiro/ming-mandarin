...
type Props = { exercises: Exercise[]; hanziCharacters: CharacterEntry[]; listeningEntries: ListeningEntry[]; scope: CurriculumScope; initialGame?: 'reto-mixto' };
export function Arcade({ hanziCharacters, listeningEntries, scope, initialGame }: Props) {
  ...
  return <div className="arcade-root" ref={rootRef}>
    <section className="game-grid ming-games-grid shell">{games.map((item, index) => <article key={item.id} data-game={item.id}>
      ...
      <div className={`game-thumbnail thumbnail-${item.kind}`} aria-hidden="true">{item.kind === 'hanzi' ? <Hanzi>字</Hanzi> : item.kind === 'story' ? <span>▤ ⌕</span> : <><Person index={index}/><span>{item.kind === 'mixed' ? '✦' : item.kind === 'conversation' ? '•••' : '＋'}</span></>}</div>
