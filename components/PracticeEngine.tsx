...
export function PracticeEngine({ exercises, title = 'Práctica guiada' }: { exercises: Exercise[]; title?: string }) {
  ...
  const usesChineseBlocks = exercise.type !== 'pinyin' && /\p{Script=Han}/u.test(exercise.answer);
  return <section className="practice-card" aria-live="polite">
    <div className="practice-top"><div><p className="eyebrow">SESIÓN · {index + 1}/{exercises.length}</p><h2><Hanzi>{title}</Hanzi></h2></div><span className="difficulty">Dificultad {exercise.difficulty}/5</span></div>
    ...
    {exercise.type === 'order' && <><div className="token-bank">{exercise.options?.map((option, optionIndex) => <button key={`${option}-${optionIndex}`} type="button" onClick={() => addToken(option)}><Hanzi>{option}</Hanzi></button>)}</div><div className="answer-line">{ordered.length ? ordered.map((item, i) => <span className="font-hanzi" lang="zh-CN" key={`${item}-${i}`}>{item}</span>) : <em>Toca los bloques en el orden correcto</em>}<button type="button" className="clear-button" onClick={() => setAnswer('')}>Borrar</button></div></>}
    {exercise.options && exercise.type !== 'order' ? <div className="option-grid">{exercise.options.map((option) => <button className={answer === option.replace(/[。？?]/g, '') ? 'selected' : ''} key={option} type="button" onClick={() => choose(option)}><Hanzi>{option}</Hanzi></button>)}</div> : exercise.type !== 'order' && (usesChineseBlocks ? <AnswerBlocks key={exercise.id} blocks={[...exercise.answer.replace(/[。？！]/g,'')]} onChange={setAnswer} disabled={Boolean(result)}/> : <label className="answer-input"><span>Tu respuesta</span><input className={hanziInputClass(answer)} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder={exercise.type === 'pinyin' ? 'Ej.: zǎoshang o zao3 shang' : 'Escribe tu respuesta'} disabled={Boolean(result)} /></label>)}
    ...
    {error && <p className="form-error">{error} {error.includes('nombre') && <Link href="/login?returnTo=/lesson/1/daily">Elegir nombre</Link>}</p>}
    {result ? <div className={result.correct ? 'feedback correct' : 'feedback incorrect'}><h3><Hanzi>{result.correct ? `正确 · +${result.xp} XP` : '再试一次 · Revisemos'}</Hanzi></h3><p><b>Tu respuesta:</b> <Hanzi>{exercise.type === 'pinyin' ? <PinyinText>{result.feedback.given}</PinyinText> : result.feedback.given}</Hanzi></p><p><b>Respuesta:</b> <Hanzi>{exercise.type === 'pinyin' ? <PinyinText>{result.feedback.expected}</PinyinText> : result.feedback.expected}</Hanzi></p><p><Hanzi>{result.feedback.why}</Hanzi></p><p className="rule-note">Regla: <Hanzi>{result.feedback.rule}</Hanzi></p><small><Hanzi>{result.feedback.next}</Hanzi> · dominio {result.mastery}%</small><button className="button button-primary" type="button" onClick={next}>{index + 1 === exercises.length ? 'Repetir sesión' : 'Siguiente'} →</button></div> : <button className="button button-dark" type="button" disabled={busy} onClick={submit}>{busy ? 'Comprobando…' : 'Comprobar'}</button>}
  </section>;
}
