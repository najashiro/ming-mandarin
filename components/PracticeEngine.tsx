...
export function PracticeEngine({ exercises, title = 'Práctica guiada' }: { exercises: Exercise[]; title?: string }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if(networkFailure){const queue=JSON.parse(localStorage.getItem('ming-offline-attempts')??'[]') as unknown[];localStorage.setItem('ming-offline-attempts',JSON.stringify([...queue,payload]));const correct=exercise.type==='pinyin'?comparePinyin(answer,exercise.answer,true):normalizeAnswer(answer)===normalizeAnswer(exercise.answer);setResult({correct,mastery:0,xp:0,feedback:{given:answer,expected:exercise.answer,why:exercise.explanation,rule:exercise.rule,next:'Intento guardado en este dispositivo; se sincronizará al recuperar conexión.'}});}else setError(cause instanceof Error ? cause.message : 'No se pudo guardar.');
    }
