...
export function PracticeEngine({ exercises, title = 'Práctica guiada' }: { exercises: Exercise[]; title?: string }) {
  ...
  return <section className="practice-card" aria-live="polite">
    ...
    {exercise.type === 'order' && <><div className="token-bank">{exercise.options?.map((option, optionIndex) => <button key={`${option}-${optionIndex}`} type="button" onClick={() => addToken(option)}>{option}</button>)}</div><div className="answer-line">{ordered.length ? ordered.map((item, i) => <span key={`${item}-${i}`}>{item}</span>) : <em>Toca los bloques en el orden correcto</em>}<button type="button" className="clear-button" onClick={() => setAnswer('')}>Borrar</button></div></>}
    ...
    {result ? <div className={result.correct ? 'feedback correct' : 'feedback incorrect'}><h3>{result.correct ? `正确 · +${result.xp} XP` : '再试一次 · Revisemos'}</h3><p><b>Tu respuesta:</b> {exercise.type === 'pinyin' ? <PinyinText>{result.feedback.given}</PinyinText> : result.feedback.given}</p><p><b>Respuesta:</b> {exercise.type === 'pinyin' ? <PinyinText>{result.feedback.expected}</PinyinText> : result.feedback.expected}</p><p>{result.feedback.why}</p><p className="rule-note">Regla: {result.feedback.rule}</p><small>{result.feedback.next} · dominio {result.mastery}%</small><button className="button button-primary" type="button" onClick={next}>{index + 1 === exercises.length ? 'Repetir sesión' : 'Siguiente'} →</button></div> : <button className="button button-dark" type="button" disabled={busy} onClick={submit}>{busy ? 'Comprobando…' : 'Comprobar'}</button>}
  </section>;
}
