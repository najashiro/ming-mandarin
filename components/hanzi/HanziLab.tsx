...
function LearnPanel({ character, onMastered }: { character: CharacterEntry; onMastered: () => void }) {
  ...
  return <section className="panel hanzi-tab-panel hanzi-learn-panel"><div className="hanzi-panel-copy"><p className="eyebrow">01 · APRENDER</p><h2>Observa el carácter completo</h2><p>Usa la cuadrícula 米字格 para comparar proporción y centro. La animación respeta el orden y la dirección de los datos técnicos.</p><div className="hanzi-controls"><button type="button" onClick={() => stage.current?.animate()}>▶ Animar</button><button type="button" onClick={() => { setVisible((current) => { const next = !current; if (next) stage.current?.show(); else stage.current?.hide(); return next; }); }}>{visible ? 'Ocultar carácter' : 'Mostrar carácter'}</button><button type="button" onClick={() => stage.current?.animate()}>↻ Repetir</button></div><button className="button button-primary" type="button" onClick={onMastered}>Lo reconozco</button><ContextList character={character} /></div><HanziWriterStage ref={stage} character={character.hanzi} /></section>;
}
...
function StrokesPanel({ character, data, onMastered }: { character: CharacterEntry; data: HanziCharacterData; onMastered: () => void }) {
  ...
  function nextStep() { const next = step >= data.strokes.length ? 0 : step; stepStage.current?.animateStroke(next); setStep(next + 1); }
  return <section className="panel hanzi-tab-panel strokes-panel"><div className="hanzi-panel-heading"><div><p className="eyebrow">03 · TRAZOS</p><h2>Orden, inicio y dirección</h2></div><div className="hanzi-subtabs"><button className={mode === 'answer' ? 'selected' : ''} type="button" onClick={() => setMode('answer')}>Respuesta</button><button className={mode === 'animation' ? 'selected' : ''} type="button" onClick={() => setMode('animation')}>Animación</button><button className={mode === 'step' ? 'selected' : ''} type="button" onClick={() => setMode('step')}>Paso a paso</button><button className={mode === 'fan' ? 'selected' : ''} type="button" onClick={() => setMode('fan')}>Despiece</button></div></div>
    {mode === 'answer' && <div className="stroke-answer-layout"><HanziStrokeSvg character={character.hanzi} data={data} /><div><h3>Cómo leer la respuesta</h3><p><i className="legend-dot" /> El punto marca dónde inicia cada trazo.</p><p><i className="legend-arrow">→</i> La línea roja indica el recorrido y su flecha, la dirección.</p><ol>{directions.map((direction, index) => <li key={index}>Trazo {index + 1}: hacia {direction.label}.</li>)}</ol></div></div>}
    {mode === 'animation' && <div className="stroke-stage-layout"><HanziWriterStage ref={animationStage} character={character.hanzi} speed={speed} /><div><h3>Animación controlada</h3><div className="hanzi-controls"><button type="button" onClick={() => { setPaused(false); animationStage.current?.animate(); }}>▶ Reproducir</button><button type="button" onClick={() => { if (paused) animationStage.current?.resume(); else animationStage.current?.pause(); setPaused(!paused); }}>{paused ? 'Continuar' : 'Pausa'}</button><button type="button" onClick={() => { setPaused(false); animationStage.current?.animate(); }}>↻ Repetir</button></div><div className="speed-picker" aria-label="Velocidad">{[0.5, 0.75, 1].map((value) => <button type="button" className={speed === value ? 'selected' : ''} onClick={() => setSpeed(value)} key={value}>{value}×</button>)}</div></div></div>}
    {mode === 'step' && <div className="stroke-stage-layout"><HanziWriterStage ref={stepStage} character={character.hanzi} showCharacter={false} /><div><h3>Un trazo por vez</h3><p>Avance actual: <b>{step}</b> / {data.strokes.length}</p><div className="hanzi-controls"><button type="button" onClick={nextStep}>Mostrar siguiente</button><button type="button" onClick={() => { setStep(0); stepStage.current?.hide(); }}>Reiniciar</button></div></div></div>}
