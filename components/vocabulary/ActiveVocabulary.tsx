...
export function ActiveVocabulary({ scope, userId = 'guest' }: { scope: CurriculumScope; userId?: string }) {
  ...
  function navigateToVocabularyWord(id: string) {
    ...
    router.push(`/study/${destination}/vocabulary?${next}`, { scroll: false });
  }
  useEffect(() => {
    if (!ready || mix) return;
  ...
  return <div className="active-vocabulary shell" data-ready={ready}>
    ...
    <div className="vocabulary-toolbar">
      <div className="vocabulary-lesson-controls"><select aria-label="Lección" value={scope} onChange={e => { resetCardFaces(); const next = new URLSearchParams(params.toString()); for (const key of ['page', 'card', 'q', 'source', 'selection', 'level']) next.delete(key); router.push(`/study/${e.target.value}/vocabulary?${next}`); }}>{lessonScopes.map((s, i) => <option key={s} value={s}>{s === 'l1-l2-l3' ? 'Acumulado' : `Lección ${i + 1}`}</option>)}</select><button type="button" aria-label="Solo favoritos" title="Solo favoritos de esta lección" aria-pressed={favoritesOnly} onClick={() => change({ favorites: favoritesOnly ? '' : '1', card: '' })}>{favoritesOnly ? '★' : '☆'}</button></div>
