...
export function DialogueRoleplay() {
  ...
  return <div className="dialogue-grid"><section className="panel scene"><p className="eyebrow">PRIMER ENCUENTRO · NIVEL LIBRE</p><div className="chat-log">{turns.map((turn,i)=><div className={turn.speaker==='Tú'?'chat mine':'chat'} key={i}><small>{turn.speaker}</small><p><Hanzi>{turn.text}</Hanzi></p></div>)}<div className="chat"><small>Míng</small><p><Hanzi>{current.system}</Hanzi></p><SpeakButton text={current.system} audioSrc={audioForMandarinText(current.system)}/></div></div><label className="answer-input"><span>Tu respuesta</span><input className={hanziInputClass(answer)} value={answer} onChange={(e)=>setAnswer(e.target.value)} onKeyDown={(e)=>e.key==='Enter'&&send()} placeholder="Escribe una respuesta válida" /></label>{message&&<p className="rule-note"><Hanzi>{message}</Hanzi></p>}<button className="button button-primary" type="button" onClick={send}>Responder →</button></section><aside className="panel"><p className="eyebrow">APOYOS</p><h2>Respuestas estudiadas</h2>{['我叫…','我姓…','认识你很高兴。','我很好。','我很忙。','我不太忙。','还行。','马马虎虎。'].map(item=><button className="phrase-chip" type="button" onClick={()=>setAnswer(item.replace('…',''))} key={item}><Hanzi>{item}</Hanzi></button>)}<p className="source-note">Se aceptan múltiples respuestas cuando las fuentes ofrecen alternativas.</p></aside></div>;
}
