...
function HanziRound({ item, characters, mode, level, result, check, next }: { item: CharacterEntry; characters: CharacterEntry[]; mode: Mode; level: number; result: boolean | null; check: (ok: boolean) => void; next: () => void }) {
  ...
  const options = [...new Set([target, ...characters.filter(other => other.id !== item.id).map(other => mode === 'Radical' ? other.radical : other.hanzi)])].slice(0,4).sort();
  ...
  return <div className="hanzi-challenge"><div className="hanzi-focal">
    ...
    {result === null && !handwriting && <>{mode === 'Componentes' ? <AnswerBlocks blocks={item.components} onChange={setAnswer}/> : level === 1 ? <div className="scene-options">{options.map(option => <button key={option} aria-pressed={answer === option} onClick={() => setAnswer(option)}><Hanzi>{option}</Hanzi></button>)}</div> : <input aria-label="Respuesta Hanzi" className={hanziInputClass(answer)} value={answer} onChange={event => setAnswer(event.target.value)}/>}<button disabled={!answer.trim()} onClick={() => check(cleanChinese(answer) === cleanChinese(target))}>Comprobar</button></>}
